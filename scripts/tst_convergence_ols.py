...
# This module tests whether our test statistic is correct under OLS assumption
def consistency_test(dgp, n):
    y, x, u = dgp.generate(n)
    model_ols = sm.OLS(y, x)
    results_ols = model_ols.fit()
    yhat = results_ols.predict(x)
    resids = y/yhat
...
error_ols = IndependentNormErrorGenerator(mean_fn=lambda X: independent_squared(X), var_fn=lambda X: 2 * independent_absolute(X))


# DGP for two cases, Y = exp(2X + 1 + u)
constant_generator = ConstantGenerator(1)
x_generator = MVNDataGenerator(means=[3], sigma=[[1]])
combined_generator = CombinedDataGenerator([x_generator, constant_generator])
betas = np.array([0.5, 1])

dgp_ols = DGP(combined_generator, betas, error_ols, exponential=False)
...
for n in n_sim:
    ...
    results_ols.append(consistency_test(dgp_ols, n))
...
sns.lineplot(x=n_sim[0:len(results_ols)], y=results_ols[:,1] - betas[0], ax=ax[1], label=r'$\mathbb{E}[e^u|X] \neq 1$')
# sns.lineplot(x=n_sim[0:len(results_ppml)], y=results_ppml[:,1] - betas[0], ax=ax[1], label=r'$\mathbb{E}[e^u|X] = 1$')
